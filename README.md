...
`Terraform` `Azure AKS` `Proxmox`

**CI/CD & GitOps**
`GitHub Actions` `GitOps` `Trivy` `GHCR`

**Storage & Networking**
`SeaweedFS` `S3-compatible storage` `HAProxy` `FRR` `BGP`

**Observability & Other**
`PostgreSQL` `Cloudflare` `Let's Encrypt`
...
| [azure-aks-platform](https://github.com/pascariucosmin93/azure-aks-platform) | Production-grade AKS reference implementation — Terraform, hub-spoke network, ACR, Key Vault |
